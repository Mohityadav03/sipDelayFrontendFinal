...
const calculateControl = async (req, res) => {
  ...
  if (
    _validation({ monthlyInvestment, investmentPeriod, rateOfReturn, delay })
  ) {
  ...
  // if the values are correct then it will call the calculator function from business logic file where the actual calculation works
    ...
    let info = await serviceLogic.calculateSip({
      monthlyInvestment,
      investmentPeriod,
      rateOfReturn,
      delay,
    ...
    res.json(info); // all data here of Api call
  }
};
...
const _validation = (options) => {
  ...
  if (!(options.monthlyInvestment >= 0)) {
    ...
    //months can not be negative and zero
    iserror = true;
  }
